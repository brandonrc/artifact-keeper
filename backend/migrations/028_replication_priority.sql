...
ALTER TABLE repositories
    ADD COLUMN replication_priority replication_priority NOT NULL DEFAULT 'on_demand';

-- Extend edge_repo_assignments with per-edge priority override and schedule
ALTER TABLE edge_repo_assignments
    ADD COLUMN priority_override replication_priority,
    ADD COLUMN replication_schedule VARCHAR(100),  -- cron expression for 'scheduled' priority
    ADD COLUMN last_replicated_at TIMESTAMP WITH TIME ZONE;

-- Index for querying repos by replication priority (used by sync scheduler)
CREATE INDEX idx_repositories_replication_priority ON repositories (replication_priority)
    WHERE replication_priority != 'local_only';

-- Index for finding assignments due for scheduled replication
CREATE INDEX idx_edge_repo_assignments_schedule
